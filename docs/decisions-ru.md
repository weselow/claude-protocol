...
**Enforcement:** Хук `validate-completion.cjs` (SubagentStop) читает последнее сообщение субагента. Если это отчёт о завершении (`BEAD {ID} COMPLETE`), субагент возвращается к работе — в любом режиме разрешений, `bypassPermissions` тоже, — когда нет `Checklist:` или в нём есть незакрытый `[ ]`, когда строка `Worktree:` не указывает на существующий каталог, когда в этой рабочей копии есть незакоммиченные изменения или когда её ветки нет на origin на том же коммите. Без origin или при недоступном origin последняя проверка пропускается. Повторную остановку (`stop_hook_active`) хук пропускает, чтобы не спорить по кругу.
