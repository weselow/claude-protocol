...
**Enforcement:** Хук `validate-completion.cjs` (SubagentStop) читает последнее сообщение субагента. Отчётом о завершении его делает строка, которая начинается с `BEAD {ID} COMPLETE` (знак заголовка или жирный шрифт перед ней допустимы). Тогда субагент возвращается к работе — в любом режиме разрешений, `bypassPermissions` тоже, — когда:
- нет `Checklist:`, в нём нет пунктов или один из пунктов не отмечен (`- [ ]`, `* [ ]`, `+ [ ]`, `1. [ ]`); незакрытые пункты вне чеклиста не считаются
- строка `Worktree:` не указывает на существующую рабочую копию git (путь может быть в кавычках, с пояснением после него, на следующей строке или в записи Git Bash на Windows)
