...
**Enforcement:** Хук `validate-completion.cjs` (SubagentStop) читает последнее сообщение субагента. Отчётом о завершении его делает строка, которая начинается с `BEAD <id> COMPLETE`. Перед маркером может стоять что угодно, кроме букв и цифр (знак заголовка, жирный шрифт, обратная кавычка, эмодзи, знак цитаты); маркер посреди фразы или с заглушкой вместо идентификатора, например `{BEAD_ID}`, не считается. Тогда субагент возвращается к работе — в любом режиме разрешений, `bypassPermissions` тоже, — когда:
