# СТАНДАРТ ИМПЛЕМЕНТАЦИИ

Действует, когда ты уже пишешь код. Как работа *начинается* — понимание,
готовое или заново, план — в `pre-code-workflow.md`.

## Метрики кода

- Cyclomatic Complexity < 10
- Function length < 30 строк
- Class length < 200 строк
- Parameters < 5 (объект для >5)
- Nesting < 4 уровней

## Правило 3-х альтернатив (для архитектурных решений)

1. Придумай 3 решения
2. Выбери простейшее работающее
3. Избегай первого что пришло в голову

## Цикл проверки

После каждого блока кода: линтинг → компиляция → тесты → запуск. Не в конце
задачи, а после каждого блока — тогда падение указывает на то, что ты написал
последним.

## Self-review (после завершения задачи)

Закоммить и отдай написанный код на проверку субагенту `code-reviewer` — он
работает по навыку `bead-review` и смотрит коммиты, а не рабочий каталог.
Кроме соответствия задаче, проверка смотрит:

- Есть ли необработанные ошибки, которые молча проглатываются?
- Есть ли SQL injection, XSS или другие уязвимости на границах ввода?
- Соблюдены ли метрики (функция <30, класс <200, nesting <4)?
- Нет ли дублирования, которое стоит вынести?
- Добавлено ли логирование по триггерам из logging-standard?
- Написаны ли тесты по триггерам из tdd-workflow?
- Соответствует ли код конвенциям проекта (CLAUDE.md проекта)?

Если изменено >3 файлов или >50 строк — вызови `/simplify` для упрощения и рефакторинга.

## Запреты

- Обратная совместимость, о которой никто не просил
- Оставлять код «как референс» — удалить значит удалить
- Спешить: лучше потратить время и написать качественно, чем потом переделывать
- Гадать при неуверенности — спроси, и вместе с вопросом дай свою рекомендацию
